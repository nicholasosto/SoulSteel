export enum ECharacterPartName {
	HEAD = "Head",
	TORSO = "Torso",
	LEFT_ARM = "LeftArm",
	RIGHT_ARM = "RightArm",
	LEFT_LEG = "LeftLeg",
	RIGHT_LEG = "RightLeg",
	HUMANOID_ROOT_PART = "HumanoidRootPart",
	// etc...
}

export enum EAttachmentName {

	// Head
	Hat = "HatAttachment",
	FaceFront = "FaceFrontAttachment",
	FaceCenterAttachment = "FaceCenterAttachment",
	
	// Upper
	LeftShoulder = "LeftShoulderAttachment",
	RightShoulder = "RightShoulderAttachment",
	LeftElbow = "LeftElbowAttachment",
	RightElbow = "RightElbowAttachment",

	// Hands
	LeftWrist = "LeftWristAttachment",
	RightWrist = "RightWristAttachment",
	LeftGrip = "LeftGripAttachment",
	RightGrip = "RightGripAttachment",

	// Lower
	LeftHip = "LeftHipAttachment",
	RightHip = "RightHipAttachment",
	WaistCenter = "WaistCenterAttachment",

	// Legs
	LeftKnee = "LeftKneeAttachment",
	RightKnee = "RightKneeAttachment",
	LeftAnkle = "LeftAnkleAttachment",
	RightAnkle = "RightAnkleAttachment",
}

